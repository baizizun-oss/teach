...
from openpyxl.cell.cell import Cell  # <-- 已正确导入
import config
import json
...
def calculateScore(submitted_file_path, question_id, correct_answer_file_path):
    ...
    try:
        ...
        if not isinstance(scoring_rules, dict):
            raise ValueError("score_rules 必须是 JSON 对象（字典）")
