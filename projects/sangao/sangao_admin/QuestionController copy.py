...
class operationAddHandler(tornado.web.RequestHandler):
    def post(self):
        ...
        rule_keys = ["cell_values", "formulas", "chart", "merged_cells"]
        for key in rule_keys:
            logger.info(f"rule_score_{key}: {self.get_body_argument(f'rule_score_{key}', default='0')}")
            logger.info(f"rule_desc_{key}: {self.get_body_argument(f'rule_desc_{key}', default='0')}")
            # 检查该规则是否启用（通过 checkbox 是否提交）
