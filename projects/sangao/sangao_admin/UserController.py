...
class quickSelectHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        # tasks=sqlite3.connect("D:\\projects3\\db\\baigaopeng_myportal.db").cursor().execute("select * from task where id="+self.get_argument("id"))
        # for vo in tasks:
        # task=vo

        # print("tasks字符集：",tasks)
        # print("task字符集:",task)
        impedes = sqlite3.connect("D:\\projects3\\db\\baigaopeng_myportal.db").cursor().execute(
            "select * from impede where status = 'abled'")
        # for vo in impedes:
        # print("impedes[0]:",vo[0])
        challenges = sqlite3.connect("D:\\projects3\\db\\baigaopeng_myportal.db").cursor().execute(
...
class operationOrderHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        holidays = sqlite3.connect("D:\\projects3\\db\\baigaopeng_myportal.db").cursor().execute(
            "select * from holiday")
